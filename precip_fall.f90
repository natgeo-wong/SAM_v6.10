...
use vars
use params
use microphysics, only: qtot_sed, qice_sed, prec_accum, prec_ice_accum
implicit none
...
!bloss: used to compute individual tendency for ice sedimentation for energy budget terms.
real :: ice_frac(1:nz)
...
do j=1,ny
   do i=1,nx
      ...
      do iprec = 1,nprec
         ...
         if(do_chunked_energy_budgets.AND.(flagstat.gt.0)) then
           !bloss: accumulate sedimentation tendencies for total water and for ice
           !         so that they can be included in energy budgets
           qtot_sed(i,j,:) = qtot_sed(i,j,:) - irhoadz(1:nzm)*( fz(2:nz)-fz(1:nzm) )

           ice_frac(1:nz) = MAX(0., MIN(1., (lfac(1:nz) - fac_cond)/fac_fus ) )
           qice_sed(i,j,:) = qice_sed(i,j,:) &
                - irhoadz(1:nzm)*( ice_frac(2:nz)*fz(2:nz) - ice_frac(1:nzm)*fz(1:nzm) )

           prec_accum(i,j) = prec_accum(i,j) - fz(1)
           prec_ice_accum(i,j) = prec_ice_accum(i,j) - ice_frac(1)*fz(1)
         end if
