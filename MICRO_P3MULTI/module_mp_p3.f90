...
!BG add accumulation of microphysical process rates here [in main loop as rates are 1dim or in iice function]
 if (do_accumulate_micro_proc_rates) then
    ...
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + SUM(qchetc(:)) ! contact freezing of cloud droplets !ABS
    idx = idx + 1
       ...
       if (qc(i,k).ge.qsmall .and. t(i,k).lt.233.15) then
         ...
         !homfrz term? 
          ...
          if (do_accumulate_micro_proc_rates) then
            ...
            micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qchomi(iice_dest) ! homogeneous freezing of cloud liquid, ABS added
       ...
       if (qr(i,k).ge.qsmall .and. t(i,k).lt.233.15) then
         ...
         ! update theta. Note temperature is NOT updated here, but currently not used after
          ...
          if (do_accumulate_micro_proc_rates) then
            ...
            micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qrhomi(iice_dest) ! homogeneous freezing of rain, ABS added
