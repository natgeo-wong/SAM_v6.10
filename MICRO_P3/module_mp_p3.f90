...
!==================================================================================================!
 ...
 subroutine p3_init(lookup_file_dir,nCat,model,stat)
...
! note: Nc may be specified or predicted (set by log_predictNc)
 ...
 real, intent(inout), dimension(its:ite,kts:kte,5+4*nCat) :: mtend    ! microphysical tendencies for all microphysical fields (e.g., kg/kg/s)
 real, intent(inout), dimension(its:ite,kts:kte,5+4*nCat) :: stend    ! sedimentation tendencies for all microphysical fields (e.g., kg/kg/s)
...
!------------------------------------------------------------------------------------------!
 ...
 mtend(:,:,iqr)  = qr(:,:)
 mtend(:,:,inr)  = nr(:,:)
 mtend(:,:,iqit) = qitot(:,:,1) ! assume nCat==1
 mtend(:,:,iqir) = qirim(:,:,1)
...
qcrfrz = qinuc+qchetc+qcheti+qrhetc+qrheti !BG sum of all freezing of cloud drops and rain [mass]
ncrfrz = ninuc+nchetc+ncheti+nrhetc+nrheti !BG sum of all freezing of cloud drops and rain [number]
...
!BG add accumulation of microphysical process rates here [in main loop as rates are 1dim or in iice function]
 if (do_accumulate_micro_proc_rates) then
    ...
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qccol(1) ! collection of cloud water by ice [kg/kg/s]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qwgrth(1) ! wet growth rate [kg/kg/s]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qidep(1) ! vapor deposition on ice [kg/kg/s]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qrcol(1) ! collection of rain mass by  ice [kg/kg/s]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qinuc(1) ! deposition/condensation freezing nuc [kg/kg/s] ABS added but not sure what it is
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qisub(1) ! sublimation of ice [kg/kg/s]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qimlt(1) ! melting of ice [kg/kg/s]
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qcrfrz(1) ! freezing of cloud droplets (qinuc + qchetc + qcheti) and rain (qrhti+qrhetc) !BG
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qchetc(1) ! contact freezing of cloud droplets !ABS
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qcheti(1) ! immersion freezing of cloud droplets !ABS
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qrhetc(1) ! contact freezing of rain !ABS
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qrheti(1) ! immersion freezing of rain !ABS
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qcshd(1) ! source of rain mass due to colling/wet growth and shedding ABS added
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qcmul(1) ! rime-splintering of cloud water ABS added
    idx = idx + 1
    micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qrmul(1) ! rime-splintering of rain ABS added
    idx = idx + 1
  
    !number rates
    !idx = idx + 1
    !micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + nimlt(1) ! melting of ice [#/kg]
    !idx = idx + 1
    !micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + nisub(1) ! sublimation of ice [#/kg]
    !idx = idx + 1
    !micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + nislf(1) ! change in ice number from collection within a category [#/kg]
    !idx = idx + 1
    !micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + ncrfrz(1) ! number freezing of cloud droplets (qinuc + qchetc + qcheti) and rain (qrhti+qrhetc) !BG
endif
!SAM additions end
...
! homogeneous freezing of cloud and rain
    ...
    ! compute mean-mass ice diameters (estimated; rigorous approach to be implemented later)
       ...
       if (qc(i,k).ge.qsmall .and. t(i,k).lt.233.15) then
         ...
         !homfrz term? 
          qchomi(iice_dest) = Q_nuc*odt !bloss: Needs to be a tendency for the budget terms.
          nchomi(iice_dest) = N_nuc*odt 

          !SAM additions by ABS
          if (do_accumulate_micro_proc_rates) then
            idx = nmicro_proc-1 !!! WORKS ONLY IF qchomi is the second last processes.
            micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qchomi(1) ! homogeneous freezing of rain, ABS added

            ! zero out process rate
            qchomi(:) = 0.
            nchomi(:) = 0.
          endif
          !end SAM additions by ABS
       ...
       if (qr(i,k).ge.qsmall .and. t(i,k).lt.233.15) then
          ...
          qrhomi(iice_dest) = Q_nuc*odt !BG
          nrhomi(iice_dest) = N_nuc*odt !BG

          !SAM additions by ABS
          if (do_accumulate_micro_proc_rates) then
            idx = nmicro_proc !!! WORKS ONLY IF qrhomi is the last processes.
            micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qrhomi(1) ! homogeneous freezing of rain, ABS added
