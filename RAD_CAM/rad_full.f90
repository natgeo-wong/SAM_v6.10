subroutine rad_full()
  ...
  real(r4) qtot
  real(r4) dayy
  integer i,j,k,m,ii,jj,i1,j1,tmp_count,nrad_call, ierr
  integer iday, iday0
  ...
  if(initrad) then
     ...
     if(do_chunked_energy_budgets.AND.(.NOT.isAllocatedIndividualQrad)) then
       !bloss: allocate individual arrays with lw/sw, full sky/clear sky heating rates
	      ...
	      coszrs = coszrs_in(1) ! needed for the isccp simulator
           ...
           solinxy(i,j) = solin(1)
            ...
            if(do_chunked_energy_budgets) then
              qrad_lw(i,j,:) = qrl(1,nzm:1:-1)
              qradclr_lw(i,j,:) = (fcnl(1,nz:2:-1)-fcnl(1,nzm:1:-1))/massl(1,nzm:1:-1)/cp ! LW fluxes are upwards

              qrad_sw(i,j,:) = qrs(1,nzm:1:-1)
              qradclr_sw(i,j,:) = - (fcns(1,nz:2:-1)-fcns(1,nzm:1:-1))/massl(1,nzm:1:-1)/cp ! SW fluxes are downwards
            end if
