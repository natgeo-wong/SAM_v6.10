...
!bloss         if(masterproc) print*,'Reading step ',nsteplast,' of 2Dcom file'
         if(nsteplast.ge.nstep) then
!bloss(2020Aug): These lines seem to lead to issues when the run is restarting
!   at a time before the last 2D output was written.  I believe that they
!   caused the last line of output from the prior time to be over-written.
!!$           backspace(46)
...
!!$           read(46)       ! a bug in gfrotran compiler
         ...
         read(46) ! real(day)
         do i=1,nfields
           if(masterproc) read(46) ! name, long_name, units
           read(46) ! byte4(:)
