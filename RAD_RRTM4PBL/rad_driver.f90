MODULE rad_driver
	...
	PUBLIC :: rad_driver_rrtm, initialize_radiation, isInitialized_RadDriver, tracesini, &
		tracesupdate, read_rad, write_rad, land_frac, constructRestartFileName
...
! Tak Yamaguchi 2015/10: RADAEROS - Simple absorbing aerosol
	...
	REAL(KIND=kind_rb), ALLOCATABLE, DIMENSION(:,:), SAVE :: asym_fr  ! asymmetry parameter
        ...
        LOGICAL, SAVE :: IsInitializedRestartFilename = .false.
        character(LEN=256), SAVE :: restart_filename, restart_filename_save
...
!!$		WRITE(*,*) 'perpetual insolation = ', p_factor*scon
	...
	SUBROUTINE write_rad()
		...
		IF (masterproc) PRINT*,'Writting radiation restart file...'
                ...
                if(.NOT.IsInitializedRestartFilename) then
                  ! Build name of restart file
                  IF (restart_sep) THEN
                    ! with restart_sep == true, each restart file is tagged with the rank of the processor/core that writes it
                    index = rank
                  ELSE
                    ! with restart_sep == false, the single restart file is tagged with the total number of subdomains/cores
                    index = nSubdomains
                  END IF
                  restart_filename = TRIM(constructRestartFileName(case, caseId, index, 'bin'))
                  restart_filename_save = TRIM(constructRestartFileName(case, caseId, index, 'old'))
                  IsInitializedRestartFilename = .true.
                END if

		IF (restart_sep) THEN
                        write(*,*) rank, TRIM(restart_filename), TRIM(restart_filename_save)
                        ! first remove old restart file (if it exists)
                        call system('rm -f ' // TRIM(restart_filename_save) )

                        ! next, move last restart file to *.old
                        call system('mv '//TRIM(restart_filename)//' '//TRIM(restart_filename_save) )

                        ! now, open a new restart file
                        open(56, file = trim(restart_filename), status='unknown',form='unformatted')
			WRITE(56) nsubdomains, nzpatch, npatch_start, npatch_end
		...
		ELSE
                        if(masterproc) then
                          write(*,*) rank, TRIM(restart_filename), TRIM(restart_filename_save)
                          ! remove old restart file (if it exists)
                          call system('rm -f ' // TRIM(restart_filename_save) )
	...
	SUBROUTINE read_rad()
		...
		IF (restart_sep) THEN
                ...
                END IF
                IF (nrestart /= 2) THEN
                  filename = TRIM(constructRestartFileName(case, caseId, index, 'bin'))
                ELSE
                  filename = TRIM(constructRestartFileName(case_restart, caseId_restart, index, 'bin'))
                END IF
	...
	! --------------------------------------------------------------------------
        ...
        FUNCTION constructRestartFileName(str1, str2, index, suffix) result(name)
                IMPLICIT NONE
		CHARACTER(LEN=80), INTENT(IN) :: str1
		CHARACTER(LEN=80), INTENT(IN) :: str2
		...
		name = './RESTART/' // TRIM(str1) //'_'// TRIM(str2) //'_'// &
		       TRIM(indexChar) //'_restart_rad.'// TRIM(suffix)
!bloss		       indexChar(13-lenstr(indexChar):12) //'_restart_rad.'// TRIM(suffix)
