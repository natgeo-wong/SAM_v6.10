...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	USE grid, ONLY: &
		...
		compute_reffc, compute_reffi, & ! get effective radius with Get_reffc, Get_reffi in microphysics
                ...
                do_chunked_energy_budgets ! option for saving MSE budget diagnostics
...
!bloss(2018-02)                          rad_reffc(1:nx,1:ny,1:nzm) = reffl(1:nx,1:ny,1:nzm)
		...
		swnsxy(:, :) = NetswDownSurface(:, :)
                ...
                lwnscxy(:, :) = NetlwUpSurfaceClearSky(:, :) ! Net LW upwards
                swnscxy(:, :) = NetswDownSurfaceClearSky(:, :) ! Net SW downwards

                lwntxy(:, :) = NetlwUpToa(:, :) ! Net LW upwards
                swntxy(:, :) = NetswDownToa(:, :) ! Net SW downwards

                lwntcxy(:, :) = NetlwUpToaClearSky(:, :)  ! Net LW upwards
                swntcxy(:, :) = NetswDownToaClearSky(:, :)  ! Net SW downwards
      
		! net full sky radiative fluxes (varying in x and y)
