...
!    if(coszrs >= solarZenithCosLimit) then 
      ...
      !
      where(q_l < minMixingRatio .OR. re_l < 1.e-9) !bloss(2020-06): Restrict to re_l > 1 nanometer
        re_l = 0.
        q_l  = 0.
      end where 
      where(q_i < minMixingRatio .OR. re_i < 1.e-9) !bloss(2020-06): Restrict to re_i > 1 nanometer
        re_i = 0.
        q_i  = 0.
      end where 
...
!bloss        end do 
      ...
      if(domodis) then
        ...
        if(dosnow_radiatively_active) then
          ...
          do k = 1,nzm
            kk = nzm+1-k ! Reversing orientation of z-direction, top-down in dtau_i, re_i
            do j = 1,ny
              ii = (j-1)*nx
              dtau_i(ii+1:ii+nx,k)  = tau_067_cldice(1:nx,j,kk) + tau_067_snow(1:nx,j,kk)
              re_i(ii+1:ii+nx,k)    = tau_067_cldice(1:nx,j,kk)*rad_reffi(1:nx,j,kk) &
                                     + tau_067_snow(1:nx,j,kk)*reffs(1:nx,j,kk)
            end do
          end do
