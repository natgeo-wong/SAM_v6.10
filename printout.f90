	...
	if(.not.(OCEAN.or.LAND)) then
	 ...
	 status(1) = 1 !bloss call task_abort()
         print*, '!!!!!!!!!====================!!!!!!!!!!!!!!!!'
	endif
	...
	if(dosurface) then
        ...
        if(dosurface.and.dosfcforcing) then
          ...
          if(.NOT.dosgs) then
            !bloss: Warn user that surface fluxes will not be communicated 
            !  to model if SGS model is not enabled.
            print*, '!!!!!!!!!===== ERROR ========!!!!!!!!!!!!!!!!'
            write(*,*) 'Error: With dosgs==false, surface fluxes will not'
            write(*,*) '  be added to model momentum, energy and moisture fields.'
            write(*,*) '  If you really want to do this, disable this error in'
            write(*,*) '  in SRC/printout.f90.'
            status(1) = 1 !bloss call task_abort()
            print*, '!!!!!!!!!====================!!!!!!!!!!!!!!!!'
          end if
	...
	if(donudging_tq) print*, 'Nudging of T and Q:', donudging_tq
      ...
      if(dompi) then
        call task_bcast_integer(0,status,1)
      end if

      if(status(1).ne.0) then
        if(masterproc) write(*,*) 'Model stopping.  Search for error message(s) in text above...'
        call task_abort()
      end if
