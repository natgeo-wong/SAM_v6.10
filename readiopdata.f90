...
!     
   ...
   if(STATUS.eq.NF_NOERR) then
     allocate(tmp_aer_in(ntime,nlev), &
          AccumAerosolNumber_snd(nlev,ntime), &
          AccumAerosolMass_snd(nlev,ntime), &
          AitkenAerosolNumber_snd(nlev,ntime), &
          AitkenAerosolMass_snd(nlev,ntime), &
          STAT=status)
     IF(status.ne.0) then
   ...
   if(STATUS.eq.NF_NOERR) then
     allocate(tmp_aer_in(ntime,nlev), &
          deltaDVapor_snd(nlev,ntime), &
          deltaO18Vapor_snd(nlev,ntime), &
          deltaO17Vapor_snd(nlev,ntime), &
          deltaDVapor_ref(nlev,ntime), &
          deltaO18Vapor_ref(nlev,ntime), &
          deltaO17Vapor_ref(nlev,ntime), &
          STAT=status)
     IF(status.ne.0) then
       write(*,*) 'Could not allocate aerosol arrays in readiopdata'
       call task_abort()
